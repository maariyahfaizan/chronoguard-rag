...
from the pre-filtered relevant-passages file produced by
fetch_and_extract_streamingqa.py, instead of re-running extraction.py
against full year archives (which no longer exist on disk -- they were
deleted right after each year's extraction to keep peak disk usage bounded).
...
Input:  data/raw/streamingqa_relevant_passages.jsonl  (from fetch_and_extract_streamingqa.py)
...
def load_relevant_passages(raw_dir: Path) -> tuple[dict, list[dict]]:
    ...
    build_passage_index() produced, just sourced from the filtered file."""
    path = raw_dir / "streamingqa_relevant_passages.jsonl"
    by_doc_id = {}
    all_passages = []
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
...
def select_distractors(
    ...
    if len(in_window) < n_needed:
        ...
        # fetch_and_extract_streamingqa.py, so the data needed to satisfy
        # this widening was actually kept during extraction.
        widened = window_days
...
def build_pools(cfg: dict, sample: list[dict], by_doc_id: dict, all_passages: list[dict]) -> list[dict]:
    ...
    if skipped:
        print(f"WARNING: {len(skipped)} questions had no gold evidence in the "
              f"relevant-passages file, eval set is {len(records)}/100: {skipped}")
...
if __name__ == "__main__":
    ...
    print(f"Loaded {len(all_passages)} relevant passages.")
